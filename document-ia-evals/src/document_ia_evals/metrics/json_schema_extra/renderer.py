...
def render_results(experiment_results: Dict[str, Any]) -> None:
    ...
    for obs in observations:
        ...
        if observation_str:
            try:
                obs_data = JsonSchemaExtraObservation.model_validate_json(
                    observation_str
                )

                for field_name, metric_name, score in _extract_field_metric_scores(
                    obs_data
                ):
                    field_metric_scores_by_model[(field_name, metric_name)][
                        model_version
                    ].append(score)
    ...
    if field_metric_scores_by_model:
        ...
        for field_name, metric_name in sorted(field_metric_scores_by_model.keys()):
            row = {"Field": field_name, "Metric": metric_name}

            # Calculate mean for each model
            has_skip = False
            for model_version in sorted(all_models):
                scores = field_metric_scores_by_model[(field_name, metric_name)].get(
                    model_version, []
                )
                if scores:
        ...
        if comparison_data:
            ...
            styled_comparison_df = comparison_df.style.apply(
                highlight_skip_rows, axis=1
            )
            st.dataframe(
                styled_comparison_df, use_container_width=True, hide_index=True
            )
    ...
    for model_version in model_versions:
        ...
        with col1:
            st.metric(
                "Overall Score",
                f"{np.mean(global_scores):.3f}" if global_scores else "N/A",
            )
        with col2:
        ...
        with col4:
            if processing_times:
                st.metric(
                    "Avg Processing Time", f"{np.mean(processing_times) / 1000:.0f} s"
                )
            else:
