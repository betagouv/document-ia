...
def render_metric_selection() -> tuple[str | None, str | None]:
    ...
    if selected_metric_name:
        ...
        with st.expander("📏 Metric Details", expanded=False):
            st.markdown(
                f"**Metric Type:** {metric_info.get('metric_type', 'standard')}"
            )
            st.markdown(
                f"**Required Fields:** {', '.join(metric_info.get('require', []))}"
            )
...
def main():
    ...
    if selected_metric:
        ...
        requires_document_type = metric_info and "document_type" in metric_info.get(
            "require", []
        )
    ...
    ready_to_run = (
        selected_project_id
        and selected_metric
        and (not requires_document_type or selected_document_type)
    )
        ...
        if run_button:
            try:
                ...
                def on_progress(progress):
                    ...
                    status_text.text(
                        f"Processing task {progress.current} of {progress.total}..."
                    )
