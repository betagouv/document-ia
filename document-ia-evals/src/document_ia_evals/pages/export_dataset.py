...
def main():
    ...
    st.caption(f"Using: Label Studio URL: {config.LABEL_STUDIO_URL}")
    ...
    if selected_project_id:
        ...
        with col1:
            export_button = st.button(
                "📥 Export Tasks", type="primary", use_container_width=True
            )
        ...
        if export_button:
            try:
                with st.spinner("Fetching tasks from Label Studio..."):
                    tasks: list[dict[str, Any]] = fetch_project_tasks(
                        selected_project_id
                    )
        ...
        if (
            st.session_state.export_data is not None
            and st.session_state.export_project_id == selected_project_id
        ):
            ...
            with st.expander("📈 Dataset Statistics", expanded=False):
                ...
                completed_tasks = sum(
                    ...
                    and any(ann.get("completed_by") for ann in task["annotations"])
                )
