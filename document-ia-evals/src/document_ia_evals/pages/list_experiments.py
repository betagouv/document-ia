...
def main():
    ...
    try:
        ...
        if not experiments:
            st.info(
                "No experiments found. Run an experiment and save it to see history."
            )
            ...
            [
                {
                    ...
                    "Avg Score": f"{exp['average_score']:.3f}"
                    if exp["average_score"]
                    else "N/A",
        ...
        if hasattr(selected_indices, "selection") and selected_indices.selection.rows:
            ...
            with col1:
                if len(selected_exp_ids) == 1 and st.button(
                    "👁️ View Details", use_container_width=True
                ):
        ...
        if "view_experiment_id" in st.session_state:
            ...
            if exp_data:
                ...
                with col2:
                    st.link_button(
                        "🔍 Ouvrir dans Label Studio",
                        f"{config.LABEL_STUDIO_URL}/projects/{exp_data['project_id']}/data",
                    )
                with col3:
                ...
                with col2:
                    st.markdown(
                        f"**Created:** {format_datetime(exp_data['created_at'])}"
                    )
                with col3:
        ...
        if "delete_confirmation" in st.session_state:
            ...
            st.warning(
                f"⚠️ Are you sure you want to delete {len(exp_ids)} experiment(s)?"
            )
