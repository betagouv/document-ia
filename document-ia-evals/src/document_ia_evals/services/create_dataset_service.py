...
def process_single_file(
    ...
    try:
        ...
        result_data_for_ls: dict[str, Any] = {}

        if mode == "classification":
            if workflow_data.result.classification is not None:
                doc_type = workflow_data.result.classification.document_type
                result_data_for_ls["document_type"] = doc_type.value if hasattr(doc_type, "value") else str(doc_type)
            else:
                return FileProcessingResult(
                    file_name=uploaded_file.name,
                    success=False,
                    error="Failed to get classification result",
                    execution_id=execution_id
                )
        else:
        ...
        task_data = create_task(pdf_url=pdf_url, ground_truth=result_data_for_ls)
