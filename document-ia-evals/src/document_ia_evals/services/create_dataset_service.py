...
def process_single_file(
    ...
    try:
        ...
        task_data = create_task(
            pdf_url=pdf_url, ground_truth=result_data, content_type=content_type
        )
        ...
        return FileProcessingResult(
            file_name=uploaded_file.name, success=True, execution_id=execution_id
        )
...
def process_single_file_v2(
    ...
    """Process a single file using V2 workflow API: execute with overrides, upload to S3.
    ...
    Args:
        ...
        s3_prefix: S3 prefix path for the dataset
        bucket_name: S3 bucket name
        override: Optional workflow parameter overrides
    ...
    try:
        # Execute workflow v2 to get ground truth
        ...
        if not execution_details or execution_details.status != ExecutionStatus.SUCCESS:
            return FileProcessingResult(
                file_name=uploaded_file.name,
                success=False,
                error=f"Workflow failed with status: {execution_details.status if execution_details else 'unknown'} (execution_id: {execution_id})",
                execution_id=execution_id,
            )
        ...
        if dataset_type == "classification":
            ...
            result_data = {"document_type": classification_result.document_type.value}
        else:
            extracted_fields = workflow_data.result.extraction
            if extracted_fields is None:
                return FileProcessingResult(
...
def _file_consumer_v2(
    ...
    """Consumer function for processing files from a queue using V2 API.
...
def process_files_parallel_v2(
    ...
    """Process files in parallel using V2 API and upload to S3 with ground truth annotations.
