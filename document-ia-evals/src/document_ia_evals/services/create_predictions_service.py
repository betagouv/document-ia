    ...
    try:
        ...
        if not pdf_url:
            return TaskProcessingResult(
                task_id=task_id, success=False, error="No PDF URL found in task data"
            )
        ...
        try:
            ...
            return TaskProcessingResult(
                task_id=task_id, success=False, error=f"Failed to extract S3 URL: {e}"
            )
        ...
        return TaskProcessingResult(
            task_id=task_id, success=False, error=str(e), execution_id=execution_id
        )
...
def get_processing_statistics(results: dict[int, dict[str, Any]]) -> tuple[int, int]:
    """
...
def process_single_task_v2(
    ...
    """Process a single task using V2 API: download file, execute workflow V2, create prediction."""
    task_id = task.id
    execution_id: str | None = None
    ...
    try:
        ...
        pdf_url = task.data.get("pdf") if task.data else None
        if not pdf_url:
            pdf_url = task.data.get("pdf_url") or task.data.get("image_url")
            if not pdf_url:
                return TaskProcessingResult(
                    task_id=task_id,
                    success=False,
        ...
        response = execute_workflow_v2(
            ...
            file=file_obj,
            override=override,
            metadata={"source": "streamlit-predictions-creation-v2"},
            sync=False,
        )
        ...
        if dataset_type == "classification":
            classification_result = workflow_data.result.classification
            if classification_result is None or not classification_result.document_type:
                return TaskProcessingResult(
                    task_id=task_id,
                    success=False,
                    error="Failed to get classification result or document type",
                    execution_id=execution_id,
                )
            doc_type_val = classification_result.document_type
            if hasattr(doc_type_val, "value"):
                doc_type_val = doc_type_val.value
            annotation_data = {"document_type": doc_type_val}
        else:
            ...
            annotation_data = {
                item.name: item.value for item in properties if item.name and item.value
            }
...
def _task_consumer_v2(
    ...
    """Consumer function for processing tasks from a queue using V2 API."""
    while True:
        try:
            task = task_queue.get(block=False)
        except queue.Empty:
...
def run_workflow_on_dataset_v2(
    workflow_id: str,
    project_id: int,
    api_key: str,
    ls_client: LabelStudio,
