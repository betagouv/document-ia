...
def annotation_results_to_dict(
...
def create_task(
    ...
    task_data = {
        ...
        "pdf": pdf_url,  # Keep pdf key for compatibility
    }

    # Keep image key for compatibility
    if not is_pdf:
        task_data["image"] = pdf_url

    task: dict[str, Any] = {"data": task_data}

    # Ground truth as annotation
    if ground_truth:
        annotation_result = dict_to_annotation_result(ground_truth)
        task["annotations"] = [{"result": annotation_result, "ground_truth": True}]

    return task


def create_classification_task(
    file_url: str, content_type: str, ground_truth: dict[str, Any] | None = None
) -> dict[str, Any]:
    """Create a complete Label Studio task for classification with PDF/Image conditional display."""
    return create_task(file_url, ground_truth, content_type)
...
def generate_label_config(
    model: Type[BaseModel], title: str = "Document Extraction"
) -> str:
...
def create_label_studio_project(
    ...
    label_config = generate_label_config(model_class, title=f"Extraction {schema.name}")
    ...
    target_storage = project.connect_s3_export_storage(  # type: ignore
        **s3_params, prefix=f"{normalized_prefix}/target"
    )
...
def create_label_studio_classification_project(
    ...
    # XML configuration with PDF / Image conditional display and document_type field
    label_config = """<View>
