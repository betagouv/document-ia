...
execution_id_var: ContextVar[Optional[str]] = ContextVar(
    "task_scheduler_execution_id", default=None
)
...
class TaskSchedulerAggregatorHandler(Handler):
...
def setup_logging_worker() -> None:
    """Add (idempotently) the aggregation handler to the task scheduler's root logger."""
    root = logging.getLogger()
    for h in root.handlers:
        if isinstance(h, TaskSchedulerAggregatorHandler):
            return
    handler = TaskSchedulerAggregatorHandler()
