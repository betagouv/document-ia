...
class MarkerHttpHttpOcrService(BaseHttpOCRService[MarkerSettings]):
    def __init__(
        self,
        config: MarkerSettings = marker_settings,
        timeout: int = 60,
    ...
    def get_api_key(self) -> str:
        if self.config.MARKER_API_KEY is None:
            raise HTTPOCRMissConfigurationException("Marker")
        return self.config.MARKER_API_KEY.get_secret_value()
