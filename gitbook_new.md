...
Liste de tous les **workflows disponibles** (et leurs paramètres) en environnement de sandbox :

🔗 <https://github.com/betagouv/document-ia/document-ia-infra/data/workflow/data/workflows.yaml>

_Note : Il est également possible d'obtenir la liste des workflows via l'endpoint `GET /api/v2/workflows/`._
...
🔗 [https://api.sandbox.document-ia.beta.gouv.fr/redoc](https://api.sandbox.document-ia.beta.gouv.fr/redoc)
...
### Exécution des workflows (Endpoints v2)
...
Une documentation détaillée sur l'exécution des workflows V2 est disponible ici :
🔗 <https://github.com/betagouv/document-ia/document-ia-api/docs/WORKFLOW_V2.md>
...
* **synchrone** via la route `POST /api/v2/workflows/{workflow_id}/execute-sync`. Le résultat de l’exécution est dans ce cas directement disponible dans la réponse de la requête.
  **Attention :** Le fonctionnement synchrone décrit n'est possible qu'en expérimentation en sandbox. En production, seul le fonctionnement asynchrone est disponible et recommandé.
...
{% hint style="info" icon="lightbulb" %}
L’utilisation de ce workflow dans le cadre d’appel API synchrone est recommandé en sandbox (faible latence : \~500ms). En production, veuillez utiliser l'appel asynchrone.
{% endhint %}
